...
const createTransaction = async (req, res, next) => {
  try {
    ...
    const httpError = createHttpError(error.statusCode, `Error creating transaction - ${error.message}`)
    next(httpError)
  }
}
...
const updateTransaction = async (req, res, next) => {
  try {
    ...
    const httpError = createHttpError(error.statusCode, `Error updating transaction - ${error.message}`)
    next(httpError)
  }
}
...
const deleteTransaction = async (req, res, next) => {
  try {
    ...
    const httpError = createHttpError(error.statusCode, `Error deleting transaction - ${error.message}`)
    next(httpError)
  }
}
